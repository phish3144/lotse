...
| Dateien deuten (PDF, Text, Markdown, CSV, JSON) | vom Nutzer priorisiert | Aufnahmetest: Frage 1 ja (das Rad nicht neu erfinden – wer ein Angebot oder eine Anleitung wiederfindet, muss sie nicht noch einmal durchlesen), Frage 2 ja (keine Pflege: es gibt keine Sammlung, keinen Index, keine Schleife über Ordner; jede Datei wird einzeln hergegeben und danach vergessen), Frage 3 nein. Die Zusage »der Beobachter liest keine Inhalte« bleibt unangetastet — das hier ist der andere Fall und heißt deshalb auch anders. Ausschlussliste (`.env`, Schlüssel, Zertifikate) gilt auch, wenn jemand die Datei ausdrücklich wählt. |
