...
|---|---|---|
| 2026-09-12 | Lizenz: **AGPL-3.0-only**. Das Repository bleibt öffentlich. | Die realistische Bedrohung ist nicht, dass jemand Lotse selbst baut – das darf er bei Open Core ohnehin. Sie ist, dass jemand eine geschlossene Fassung als Dienst anbietet; genau das verhindert §13 der AGPL. Verkauft werden ohnehin die Dienste (gehosteter Sync, gehosteter KI-Zugang), nicht der Client: ein Kopierschutz in einer lokal laufenden App ist ein Patch entfernt, und Lotse hat bewusst keinen Server, der Klartext sieht. Umgekehrt trägt die Offenheit das Verkaufsargument – „wir können deine Daten nicht lesen" ist nur so viel wert, wie es überprüfbar ist. `-only` statt `-or-later`, damit eine künftige AGPLv4 nicht automatisch gilt und der kommerzielle Doppelvertrieb als Alleinurheber erhalten bleibt. Ausgeführt in `BUSINESS.md` 3. |
| 2026-09-12 | Die Landing Page läuft unter `lotse.sanctora.eu`, ausgeliefert weiterhin von GitHub Pages. | Ein CNAME statt eines Plattformwechsels: die Auslieferungskette bleibt, wie sie ist, und `site/` enthält keine absoluten Pfade, die eine Basis voraussetzen. Dass die Seite öffentlich ist, erzwingt nicht, dass der Code es ist – `site/` ließe sich jederzeit in ein eigenes Repo ziehen, der DNS-Eintrag bliebe gültig. |
...
| 2026-09-06 | Lizenz noch nicht festgelegt, Repo bleibt privat. *(überholt durch den Eintrag vom 2026-09-12)* | Entscheidung vor erster Veröffentlichung, siehe `BUSINESS.md`. |
